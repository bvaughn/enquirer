...
class BooleanPrompt extends Prompt {
  ...
  async render() {
    let { input, size, submitted } = this.state;
    let { hideAfterSubmit, hideHelp, hideHint, hideOutput } = this.options;

    let prefix = await this.prefix();
    let sep = ""; // await this.separator();
    let msg = await this.message();
    let hint = hideHint ? "" : this.styles.muted(this.default);
    ...
    if (help && !hideHelp && !promptLine.includes(help)) output += " " + help;
    if (!hideOutput) promptLine += " " + output;

    this.clear(size);
    if (submitted && hideAfterSubmit) {
      // Don't render after prompt has been submitted
    } else {
